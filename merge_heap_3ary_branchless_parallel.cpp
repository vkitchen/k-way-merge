	...
	class heap {
		private:
			...
			void sift_down(size_t position) {
				...
				while (true) {
					size_t first_child = position * 3 + 1;
					...
					size_t last_child = first_child + 2;
					if (last_child >= size)
						last_child = size - 1;
...
bool MergeHeap3aryBranchlessParallel::merge(struct test *t, int n) {
